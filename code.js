...
function setProduit(data) {
    ...
    div4.append(select)
    let option1 = new createElement(null, null, null, null, null, `${data.lenses}`, `${data.lenses[0]}`).createOption()
    select.append(option1)
    let option2 = new createElement(null, null, null, null, null,  `${data.lenses}`, `${data.lenses[1]}`).createOption()
    select.append(option2)
    let option3 = new createElement(null, null, null, null, null, `${data.lenses}`, `${data.lenses[2]}`).createOption()
    select.append(option3)
    let a = new createElement('btn btn-warning', 'style', 'position: absolute; top: 15px; right: 15px;', 'Ajouter au panier +',  'data-panier', `${data._id}`).createLink()
    div4.append(a)
}
